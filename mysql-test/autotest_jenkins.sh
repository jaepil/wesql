...
if [ x"$test_type" = x"normal" ]; then
    ...
    extra_mtr_option="--suite=rds,smartengine,smartengine_binlog,smartengine_main,smartengine_binlog_gtid,smartengine_binlog_nogtid,smartengine_sysvars,rpl_nogtid"
elif [ x"$test_type" = x"s3" ]; then
    extra_mtr_option="--suite=rpl,smartengine_rpl,smartengine_rpl_basic,smartengine_rpl_gtid,smartengine_rpl_nogtid"
else
...
RUNMTR="./mysql-test-run.pl --report-unstable-tests --sanitize --timer --force --parallel=$para --comment=ps_row --ps-protocol --skip-ndb --skip-test-list=collections/disabled-daily.list --nounit-tests --max-connections=2048 --mysqld=--smartengine=1 --mysqld=--recovery-inconsistency-check=off $extra_mtr_option"
