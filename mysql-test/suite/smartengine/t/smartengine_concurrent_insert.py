...
Example Usage (in Mysql Test Framework):
  ...
  let $exec = python suite/smartengine/t/smartengine_concurrent_insert.py \
...
if __name__ == '__main__':
  if len(sys.argv) != 8:
    print ("Usage: smartengine_concurrent_insert.py user host port db_name table_name num_inserts num_threads")
    sys.exit(1)
  ...
  for w in workers:
    w.join()
    if w.exception:
      print ("Worker hit an exception: ", w.exception)
      worker_failed = True
