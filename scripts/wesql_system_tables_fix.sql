...
SET @cmd = "INSERT INTO consensus_applier_info
  ...
  where Channel_name='consensus_replication_applier' and Consensus_apply_index > 0";
SET @str=IF(@have_consensus_index <> 0 AND @have_consensus_replication = 1, @cmd, 'SET @dummy = 0');
PREPARE stmt FROM @str;
EXECUTE stmt;
DROP PREPARE stmt;
SET @cmd = "DELETE FROM slave_relay_log_info where Channel_name='consensus_replication_applier'";
SET @str=IF(@have_consensus_index <> 0, @cmd, 'SET @dummy = 0');
PREPARE stmt FROM @str;
EXECUTE stmt;
DROP PREPARE stmt;
SET @cmd = "DELETE FROM slave_master_info where Channel_name='consensus_replication_applier'";
SET @str=IF(@have_consensus_index <> 0, @cmd, 'SET @dummy = 0');
PREPARE stmt FROM @str;
EXECUTE stmt;
DROP PREPARE stmt;
SET @cmd = "DELETE FROM consensus_applier_info;";
SET @str=IF(@have_consensus_replication = 0, @cmd, 'SET @dummy = 0');
PREPARE stmt FROM @str;
EXECUTE stmt;
DROP PREPARE stmt;
COMMIT;
...
EXECUTE stmt;
DROP PREPARE stmt;
SET @cmd = "DELETE FROM slave_worker_info where Channel_name='consensus_replication_applier'";
SET @str=IF(@have_consensus_index <> 0, @cmd, 'SET @dummy = 0');
PREPARE stmt FROM @str;
EXECUTE stmt;
DROP PREPARE stmt;
