...
//    str.append(buf);
//  }
//  __SE_LOG(INFO, "%s", str.c_str());
//}

//void LRUCacheShard::print_shard_info() {
//  SE_LOG(INFO, "cache.shard.info: usage info", K(usage_), K(lru_usage_),
...
//  }
//
//  SE_LOG(INFO, "cache.shard.info: block count", K(block_cnt), K(no_record));
//}
...
void RowCacheRWMutex::read_lock() {  
  uint64_t cpu_id = smartengine::port::PhysicalCoreID();
...
//      SE_LOG(WARN, "alloc memory failed", K(ret), K(sizeof(SeqRange)));
//    }
//  }
...
int NewRowCache(int64_t capacity, std::shared_ptr<RowCache> &row_cache)
{
  int ret = Status::kOk;
  RowCache *row_cache_ptr = nullptr;

  if (IS_NULL(row_cache_ptr = new RowCache())) {
    ret = Status::kMemoryLimit;
    SE_LOG(WARN, "fail to allocate memory for row cache", K(ret));
  } else if (FAILED(row_cache_ptr->init(capacity))) {
    SE_LOG(WARN, "fail to init row cache", K(ret), K(capacity));
  } else {
    row_cache.reset(row_cache_ptr);

    SE_LOG(INFO, "success to initialize row cache", K(capacity));
  }

  if (FAILED(ret)) {
    if (nullptr != row_cache_ptr) {
      delete row_cache_ptr;
      row_cache_ptr = nullptr;
    }
  }
