...
int CheckpointImpl::create_temp_directory(const std::string &checkpoint_dir) {
  ...
  if (s.ok()) {
    ...
    SE_LOG(ERROR, "Directory exists.", K(ret));
    return ret; 
  } else if(!s.IsNotFound()) {
    ret = s.code();
    SE_LOG(ERROR, "IO error of checkpoint path.", K(ret));
    return ret; 
  }
  ...
  if (!s.ok()) {
    ret = s.code();
    SE_LOG(ERROR, "Create the tmperary directory failed.", K(ret));
  }
...
int CheckpointImpl::checkpoint_phase_one(const std::string &checkpoint_dir) {
  ...
  if (!s.ok()) {
    ret = s.code();
    SE_LOG(ERROR, "Flush memtable of checkpoint failed", K(ret));
    return ret;
  }
  ...
  for (auto &file_name : subchildren) {
    ...
    if (!ok) {
      // ignore those files
      __SE_LOG(WARN, "Parse file name %s failed.", file_name.c_str());
      continue;
    }
    ...
    if (type == kTableFile ||
      ...
      __SE_LOG(INFO, "Hard linking %s", file_name.c_str());
      s = db_->GetEnv()->LinkFile(db_->GetName() + "/" + file_name, 
                                  tmp_path + "/" + file_name);
      if (!s.ok()) {
        ret = s.code();
        SE_LOG(ERROR, "Hard link file failed.", K(ret));
        break;
      }
    }
  }
...
int CheckpointImpl::checkpoint_phase_two(const std::string &checkpoint_dir,
  ...
  if (dest_fd < 0) {
      ret = errno;
      __SE_LOG(ERROR, "smartengine: create extent incremental file failed errno %d", ret);
      return ret;
  }
  ...
  close(dest_fd);
  if (Status::kOk != ret) {
    SE_LOG(ERROR, "Stream the logged extents failed", K(ret));
    return ret;
  }
...
int CheckpointImpl::checkpoint_phase_three(const std::string &checkpoint_dir) {
  ...
  if (!s.ok()) {
    ret = s.code();
    SE_LOG(ERROR, "Get all wal files failed.", K(ret));
    return ret;
  }
  ...
  for (size_t i = 0; s.ok() && i < wal_size; ++i) {
    if (i + 1 == wal_size) {
      ...
      __SE_LOG(INFO, "Hard Linking lasting wal file %s\n", old_name.c_str());
      ...
      if (live_wal_files[i]->SizeFileBytes() > 
        ...
        if (!s.ok()) {
          __SE_LOG(ERROR, "Copy the last block of wal file failed.");
        }
      }
      break;
    }
    __SE_LOG(INFO, "Hard Linking %s\n",
  ...
  if (!s.ok()) {
    ...
    __SE_LOG(INFO, "Snapshot sequence number: %" PRIu64, sequence_number);
  }
  return ret;
}
...
int CheckpointImpl::checkpoint_phase_four(const std::string &checkpoint_dir,
  ...
  for (auto &file_name : subchildren) {
    if (0 == file_name.compare(0, 8, "MANIFEST")) { 
      ...
      if (!s.ok()) {
        ret = s.code();
        SE_LOG(ERROR, "Get old manifest file size failed", K(ret));
        ...
        if (manifest_file_size > old_manifest_file_size) {
          ...
          if (!s.ok()) {
            ret = s.code();
            SE_LOG(ERROR, "copy the incremental manifest file failed", K(ret));
            remove_temp_directory(tmp_path);
            remove_temp_directory(old_manifest_dir);
            remove_temp_directory(last_manifest_dir);
          }
  ...
  if (Status::kOk == ret) {
    ...
    for (auto &file_name : subchildren) {
      ...
      if (found != std::string::npos && 
        ...
        if (!s.ok()) {
          ret = s.code();
          SE_LOG(ERROR, "Copy the the last wal file failed", K(ret));
          remove_temp_directory(tmp_path);
        }
        break;
      }
...
//        unique_ptr<WritableFile> destfile;
        ...
        if (!s.ok()) {
          ret = s.code();
          SE_LOG(ERROR, "Rename the the last wal file failed", K(ret));
        }
      }
    }
  }
...
int CheckpointImpl::manual_checkpoint(const std::string &checkpoint_dir,
  ...
  switch (phase) {
    ...
    default:
      ret = Status::kInvalidArgument;
      SE_LOG(ERROR, "Input a wrong phase number of checkpoint", K(ret));
      return ret; 
  } 
}
...
Status CheckpointImpl::CreateCheckpoint(const std::string& checkpoint_dir,
  ...
  size_t wal_size = live_wal_files.size();
  __SE_LOG(INFO, "Started the snapshot process -- creating snapshot in directory %s", checkpoint_dir.c_str());
  ...
  for (size_t i = 0; s.ok() && i < live_files.size(); ++i) {
    ...
    if ((type == kTableFile) && same_fs) {
      __SE_LOG(INFO, "Hard Linking %s", src_fname.c_str());
    ...
    if ((type != kTableFile) || (!same_fs)) {
      __SE_LOG(INFO, "Copying %s", src_fname.c_str());
  ...
  __SE_LOG(INFO, "Number of log files %" ROCKSDB_PRIszt, live_wal_files.size());
  ...
  for (size_t i = 0; s.ok() && i < wal_size; ++i) {
    if ((live_wal_files[i]->Type() == kAliveLogFile) &&
      ...
      if (i + 1 == wal_size) {
        __SE_LOG(INFO, "Copying %s", live_wal_files[i]->PathName().c_str());
      ...
      if (same_fs) {
        ...
        __SE_LOG(INFO, "Hard Linking %s", live_wal_files[i]->PathName().c_str());
        s = db_->GetEnv()->LinkFile(
            db_options.wal_dir + live_wal_files[i]->PathName(),
            full_private_path + live_wal_files[i]->PathName());
        if (s.IsNotSupported()) {
...
//    unique_ptr<Directory> checkpoint_directory;
  ...
  if (!s.ok()) {
    ...
    __SE_LOG(INFO, "Snapshot failed -- %s", s.ToString().c_str());
    // we have to delete the dir and all its children
    std::vector<std::string> subchildren;
    ...
    for (auto& subchild : subchildren) {
      ...
      __SE_LOG(INFO, "Delete file %s -- %s",
                     subchild_path.c_str(), s1.ToString().c_str());
    }
    // finally delete the private dir
    Status s1 = db_->GetEnv()->DeleteDir(full_private_path);
    __SE_LOG(INFO, "Delete dir %s -- %s",
                   full_private_path.c_str(), s1.ToString().c_str());
    return s;
  }

  // here we know that we succeeded and installed the new snapshot
  __SE_LOG(INFO, "Snapshot DONE. All is good");
  __SE_LOG(INFO, "Snapshot sequence number: %" PRIu64,
                 sequence_number);
