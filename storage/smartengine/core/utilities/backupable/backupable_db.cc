...
void BackupableDBOptions::Dump() const {
  __SE_LOG(INFO, "               Options.backup_dir: %s",
                 backup_dir.c_str());
  __SE_LOG(INFO, "               Options.backup_env: %p", backup_env);
  __SE_LOG(INFO, "        Options.share_table_files: %d",
                 static_cast<int>(share_table_files));
  __SE_LOG(INFO, "                     Options.sync: %d",
                 static_cast<int>(sync));
  __SE_LOG(INFO, "         Options.destroy_old_data: %d",
                 static_cast<int>(destroy_old_data));
  __SE_LOG(INFO, "         Options.backup_log_files: %d",
                 static_cast<int>(backup_log_files));
  __SE_LOG(INFO, "        Options.backup_rate_limit: %" PRIu64,
                 backup_rate_limit);
  __SE_LOG(INFO, "       Options.restore_rate_limit: %" PRIu64,
                 restore_rate_limit);
  __SE_LOG(INFO, "Options.max_background_operations: %d",
...
Status BackupEngineImpl::Initialize() {
  ...
  initialized_ = true;
  if (read_only_) {
    __SE_LOG(INFO, "Starting read_only backup engine");
  }
  options_.Dump();
  ...
  for (auto& file : backup_meta_files) {
    ...
    __SE_LOG(INFO, "Detected backup %s", file.c_str());
  ...
  if (options_.destroy_old_data) {  // Destroy old data
    ...
    for (auto backup_iter = backups_.rbegin();
      ...
      Status s = backup_iter->second->LoadFromFile(options_.backup_dir,
                                                   abs_path_to_size);
      if (s.IsCorruption()) {
        __SE_LOG(INFO, "Backup %u corrupted -- %s",
        ...
        __SE_LOG(INFO, "Loading backup %" PRIu32 " OK:\n%s",
                       backup_iter->first,
  ...
  __SE_LOG(INFO, "Latest backup is %u", latest_backup_id_);
  ...
  __SE_LOG(INFO, "Initialized BackupEngine");
...
Status BackupEngineImpl::CreateNewBackupWithMetadata(
  ...
  __SE_LOG(INFO, "add files for backup done, wait finish.");
  Status item_status;
...
//    unique_ptr<Directory> backup_private_directory;
  ...
  if (!s.ok()) {
    ...
    __SE_LOG(INFO, "Backup failed -- %s",
                   s.ToString().c_str());
    __SE_LOG(INFO, "Backup Statistics %s\n",
  ...
  __SE_LOG(INFO, "Backup DONE. All is good");

  // backup_speed is in byte/second
  double backup_speed = new_backup->GetSize() / (1.048576 * backup_time);
  __SE_LOG(INFO, "Backup number of files: %u",
                 new_backup->GetNumberFiles());
  char human_size[16];
  AppendHumanBytes(new_backup->GetSize(), human_size, sizeof(human_size));
  __SE_LOG(INFO, "Backup size: %s", human_size);
  __SE_LOG(INFO, "Backup time: %" PRIu64 " microseconds",
                 backup_time);
  __SE_LOG(INFO, "Backup speed: %.3f MB/s", backup_speed);
  __SE_LOG(INFO, "Backup Statistics %s",
                 backup_statistics_.ToString().c_str());
  return s;
}
...
Status BackupEngineImpl::PurgeOldBackups(uint32_t num_backups_to_keep) {
  assert(initialized_);
  assert(!read_only_);
  __SE_LOG(INFO, "Purging old backups, keeping %u",
                 num_backups_to_keep);
...
Status BackupEngineImpl::DeleteBackup(BackupID backup_id) {
  assert(initialized_);
  assert(!read_only_);
  __SE_LOG(INFO, "Deleting backup %u", backup_id);
  ...
  for (auto& itr : backuped_file_infos_) {
    if (itr.second->refs == 0) {
      ...
      __SE_LOG(INFO, "Deleting %s -- %s", itr.first.c_str(),
                     s.ToString().c_str());
      to_delete.push_back(itr.first);
    }
  }
  ...
  __SE_LOG(INFO, "Deleting private dir %s -- %s",
...
Status BackupEngineImpl::RestoreDBFromBackup(
  ...
  __SE_LOG(INFO, "Restoring backup id %u\n", backup_id);
  __SE_LOG(INFO, "keep_log_files: %d\n",
  ...
  for (const auto& file_info : backup->GetFiles()) {
    ...
    __SE_LOG(INFO, "Restoring %s to %s\n", file.c_str(),
                   dst.c_str());
  ...
  __SE_LOG(INFO, "Restoring done -- %s\n",
                 s.ToString().c_str());
  return s;
}
...
Status BackupEngineImpl::VerifyBackup(BackupID backup_id) {
  ...
  __SE_LOG(INFO, "Verifying backup id %u\n", backup_id);
...
Status BackupEngineImpl::AddBackupFileWorkItem(
  ...
  if (!contents.empty() || need_to_copy) {
    __SE_LOG(INFO, "Copying %s to %s", fname.c_str(),
                   dst_path_tmp.c_str());
    CopyOrCreateWorkItem copy_or_create_work_item(
...
Status BackupEngineImpl::GarbageCollect() {
  assert(!read_only_);
  __SE_LOG(INFO, "Starting garbage collection");
  ...
  for (auto& child : private_children) {
    ...
    __SE_LOG(INFO, "Deleting dir %s -- %s",
