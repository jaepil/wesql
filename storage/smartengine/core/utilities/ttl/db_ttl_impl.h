...
#include "smartengine/utilities/utility_db.h"
...
class DBWithTTLImpl : public DBWithTTL {
 public:
  ...
  using smartengine::util::StackableDB::KeyMayExist;
  ...
  using smartengine::util::StackableDB::NewIterator;
 ...
 public:
  ...
  virtual bool FullMergeV2(const MergeOperationInput& merge_in,
    ...
    if (merge_in.existing_value && merge_in.existing_value->size() < ts_len) {
      __SE_LOG(ERROR, "Error: Could not remove timestamp from existing value.");
      return false;
    }
    ...
    for (const auto& operand : merge_in.operand_list) {
      if (operand.size() < ts_len) {
        __SE_LOG(ERROR, "Error: Could not remove timestamp from operand value.");
        return false;
      }
    ...
    if (!env_->GetCurrentTime(&curtime).ok()) {
      __SE_LOG(ERROR, "Error: Could not get current time to be attached internally to the new value.");
      return false;
    } else {
  ...
  virtual bool PartialMergeMulti(const common::Slice& key,
    ...
    for (const auto& operand : operand_list) {
      if (operand.size() < ts_len) {
        __SE_LOG(ERROR, "Error: Could not remove timestamp from value.");
        return false;
      }
