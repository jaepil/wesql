...
Status BlockCacheTier::Open() {
  ...
  assert(status.ok());
  if (!status.ok()) {
    __SE_LOG(ERROR, "Invalid block cache options");
    return status;
  }
  ...
  if (!status.ok()) {
    __SE_LOG(ERROR, "Error creating directory %s. %s", opt_.path.c_str(),
  ...
  status = NewCacheFile();
  if (!status.ok()) {
    __SE_LOG(ERROR, "Error creating new file %s. %s", opt_.path.c_str(),
                  status.ToString().c_str());
    return status;
  }
...
Status BlockCacheTier::CleanupCacheFolder(const std::string& folder) {
  ...
  if (!status.ok()) {
    __SE_LOG(ERROR, "Error getting files for %s. %s", folder.c_str(),
                  status.ToString().c_str());
    return status;
  }
  ...
  for (auto file : files) {
    if (IsCacheFile(file)) {
      // cache file
      __SE_LOG(INFO, "Removing file %s.", file.c_str());
      status = opt_.env->DeleteFile(folder + "/" + file);
      if (!status.ok()) {
        __SE_LOG(ERROR, "Error deleting file %s. %s", file.c_str(),
                      status.ToString().c_str());
        return status;
      }
    } else {
      __SE_LOG(DEBUG, "Skipping file %s", file.c_str());
    }
  }
  return Status::OK();
}
...
Status BlockCacheTier::InsertImpl(const Slice& key, const Slice& data) {
  ...
  while (!cache_file_->Append(key, data, &lba)) {
    if (!cache_file_->Eof()) {
      __SE_LOG(DEBUG, "Error inserting to cache file %d", cache_file_->cacheid());
...
Status BlockCacheTier::NewCacheFile() {
  ...
  __SE_LOG(INFO, "Created cache file %d", writer_cache_id_);
  ...
  // assert(status);
  if (!status) {
    __SE_LOG(ERROR, "Error inserting to metadata");
