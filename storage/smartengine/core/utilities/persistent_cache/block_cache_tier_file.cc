...
bool RandomAccessCacheFile::OpenImpl(const bool enable_direct_reads) {
  ...
  __SE_LOG(DEBUG, "Opening cache file %s", Path().c_str());
...
//  std::unique_ptr<RandomAccessFile> file;
  ...
  if (!status.ok()) {
    __SE_LOG(ERROR, "Error opening random access file %s. %s", Path().c_str(), status.ToString().c_str());
    return false;
  }
...
bool RandomAccessCacheFile::Read(const LBA& lba, Slice* key, Slice* val,
  ...
  if (!s.ok()) {
    __SE_LOG(ERROR, "Error reading from file %s. status: %s", Path().c_str(),
                  s.ToString().c_str());
    return false;
  }
...
bool RandomAccessCacheFile::ParseRec(const LBA& lba, Slice* key, Slice* val,
  ...
  if (!rec.Deserialize(data)) {
    ...
    __SE_LOG(ERROR, "Error de-serializing record from file %s off %d",
                  Path().c_str(), lba.off_);
    return false;
  }
...
bool WriteableCacheFile::Create(const bool enable_direct_writes,
  ...
  __SE_LOG(DEBUG, "Creating new cache %s (max size is %d B)", Path().c_str(), max_size_);

  Status s = env_->FileExists(Path());
  if (s.ok()) {
    __SE_LOG(WARN, "File %s already exists. %s", Path().c_str(), s.ToString().c_str());
  }

  s = NewWritableCacheFile(env_, Path(), file_);
  if (!s.ok()) {
    __SE_LOG(WARN, "Unable to create file %s. %s", Path().c_str(), s.ToString().c_str());
    return false;
  }
...
bool WriteableCacheFile::Append(const Slice& key, const Slice& val, LBA* lba) {
  ...
  if (!ExpandBuffer(rec_size)) {
    ...
    __SE_LOG(DEBUG, "Error expanding buffers. size=%d", rec_size);
    return false;
  }
...
bool WriteableCacheFile::ExpandBuffer(const size_t size) {
  ...
  while (free < size) {
    ...
    if (!buf) {
      __SE_LOG(DEBUG, "Unable to allocate buffers");
      return false;
    }
...
bool WriteableCacheFile::ReadBuffer(const LBA& lba, Slice* key, Slice* block,
  ...
  if (!ReadBuffer(lba, scratch)) {
    __SE_LOG(ERROR, "Error reading from buffer. cache=%d off=%d",
                  cache_id_, lba.off_);
    return false;
  }
...
void WriteableCacheFile::Close() {
  ...
  __SE_LOG(INFO, "Closing file %s. size=%d written=%d", Path().c_str(),
                size_, disk_woff_);
