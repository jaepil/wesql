...
#include "smartengine/utilities/table_properties_collectors.h"
      ...
      for (int delete_rate = 0; delete_rate <= kSample; ++delete_rate) {
        ...
        for (int i = 0; i < kPaddedWindowSize; ++i) {
          if (i % kSample < delete_rate) {
            collector->AddUserKey("hello", "smartengine", kEntryDelete, 0, 0);
            deletions++;
          } else {
            collector->AddUserKey("hello", "smartengine", kEntryPut, 0, 0);
          }
        }
