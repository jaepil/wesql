...
common::Status DeleteScheduler::DeleteFile(const std::string& file_path) {
  ...
  if (!s.ok()) {
    __SE_LOG(ERROR, "Failed to move %s to trash directory (%s)", file_path.c_str(), trash_dir_.c_str());
    s = env_->DeleteFile(file_path);
    if (s.ok() && sst_file_manager_) {
      sst_file_manager_->OnDeleteFile(file_path);
    }
...
common::Status DeleteScheduler::DeleteTrashFile(
  ...
  if (!s.ok()) {
    ...
    __SE_LOG(ERROR, "Failed to delete %s from trash -- %s", path_in_trash.c_str(), s.ToString().c_str());
    *deleted_bytes = 0;
  } else {
