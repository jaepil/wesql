...
TEST_F(ConcurrentHashMapTest, test_single_thread) {
  ...
  int i = 0;
  for (auto& entry : map) {
    SE_LOG(INFO, "map", K(entry.first), K(entry.second), K(i));
    i++;
  }
  map.clear();
  ...
  for (it = map.begin(); it != map.end(); ++it) {
    SE_LOG(INFO, "map", K(it->first), K(it->second));
  }
}
...
TEST_F(ConcurrentHashMapTest, test_multi_thread) {
  ...
  for (auto& entry : map) {
    SE_LOG(INFO, "map", K(entry.first), K(entry.second));
