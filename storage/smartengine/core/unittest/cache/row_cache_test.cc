...
TEST_F(RowCacheTest, lru_cache_test) {
  ...
  ASSERT_OK(Put(1, "f", "f"));
  Get(1, "a", nullptr);
  ASSERT_EQ(34, options.row_cache.get()->get_usage());
  Get(1, "b", nullptr);
  ASSERT_EQ(68, options.row_cache.get()->get_usage());
  Get(1, "c", nullptr);
  ASSERT_EQ(102, options.row_cache.get()->get_usage());
  Get(1, "d", nullptr);
  ASSERT_EQ(136, options.row_cache.get()->get_usage());
  ASSERT_EQ(cache_hit_ + 0, TestGetGlobalCount(CountPoint::ROW_CACHE_HIT));
  ...
  ASSERT_OK(Put(1, "g", "g"));
  Get(1, "e", nullptr);
  ASSERT_EQ(170, options.row_cache.get()->get_usage());
  Get(1, "f", nullptr); 
  ASSERT_EQ(204, options.row_cache.get()->get_usage());
