...
public:
  ...
  stl_adapt_allocator()
      : stl_alloc_(Mod) SE_USE_NOEXCEPT {}
  stl_adapt_allocator(AllocatorT alloc)
      :stl_alloc_(alloc) SE_USE_NOEXCEPT {}

  stl_adapt_allocator(const stl_adapt_allocator &) SE_USE_NOEXCEPT {}

  template <typename Tp1, int Mod1>
  stl_adapt_allocator(const stl_adapt_allocator<Tp1, Mod1, AllocatorT> &) SE_USE_NOEXCEPT {}


  ~stl_adapt_allocator() SE_USE_NOEXCEPT {}

  pointer address(reference x) const SE_NOEXCEPT {
    return std::addressof(x);
  }

  const_pointer address(const_reference x) const SE_NOEXCEPT {
    return std::addressof(x);
  }
  ...
  size_type max_size() const SE_USE_NOEXCEPT {
...
typedef std::basic_string<char, std::char_traits<char>,
    stl_adapt_allocator<char>> xstring;
