...
#define MOD_NEW_OBJECT(MOD, T, ...)                                            \
  ({                                                                           \
    T *__ret = nullptr;                                                        \
    void *__buf = smartengine::memory::base_memalign(alignof(std::max_align_t), sizeof(T), MOD);\
    if (nullptr != __buf) {                                                    \
...
#define MOD_DELETE_OBJECT(T, ptr)                                              \
  do {                                                                         \
    if (nullptr != ptr) {                                                      \
      ptr->~T();                                                               \
      smartengine::memory::base_memalign_free(ptr);                                \
      ptr = nullptr;                                                           \
    }                                                                          \
  } while (0)
...
#define MOD_DEFINE_UNIQUE_PTR(PTR, MOD, T, ...)                                \
  std::unique_ptr<T, ptr_delete<T>> PTR(                                   \
      new (smartengine::memory::base_malloc(sizeof(T), MOD)) T(__VA_ARGS__));
