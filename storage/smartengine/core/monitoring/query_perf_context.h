...
class QueryPerfContext {
 public:
  ...
  TimeType current(smartengine::util::Env *env) const;
  ...
  static void async_log_stats(smartengine::util::Env *env, const std::string &path,
 ...
 private:
  ...
  void dump_trace_to_str(const int64_t total_time, std::string &content);
  /**Dump detailed counter statistics to content.
  @param[in/out]  content target trace statistics buffer*/
  void dump_counter_to_str(std::string &content);
 private:
...
inline void query_trace_reset() {
  // opt_enable_count_: smartengine has been initialized
...
#define DECLARE_VAR_(line, trace_point) \
  smartengine::monitor::TraceGuard perf_guard##line(trace_point);
...
#define QUERY_COUNT(count_point) smartengine::monitor::query_count(count_point);
#define QUERY_COUNT_ADD(count_point, delta) \
  smartengine::monitor::query_count_add(count_point, delta);
#define QUERY_COUNT_SHARD(count_point, shard)
#define QUERY_TRACE_RESET() smartengine::monitor::query_trace_reset();
#define QUERY_TRACE_SCOPE(trace_point) \
  smartengine::monitor::TraceGuard COMBINE(perf_guard, __LINE__)(trace_point);
#define QUERY_TRACE_BEGIN(trace_point) \
  smartengine::monitor::query_trace_begin(trace_point);
#define QUERY_TRACE_END() smartengine::monitor::query_trace_end();
#define QUERY_TRACE_FINISH(query, query_length) \
  smartengine::monitor::query_trace_finish(query, query_length);
#else
