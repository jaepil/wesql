...
int DataFile::create(const CreateDataFileArgs &arg)
{
  ...
  if (UNLIKELY(is_inited_)) {
    ...
    SE_LOG(WARN, "DataFile has been inited", K(ret), K(arg));
  } else if (UNLIKELY(!arg.is_valid())) {
    ret = Status::kInvalidArgument;
    SE_LOG(WARN, "invalid argument", K(ret), K(arg));
  } else if (SUCCED(env_->FileExists(arg.data_file_path_).code())) {
    ret = Status::kErrorUnexpected;
    SE_LOG(ERROR, "unexpected error, data file should not exist", K(ret), K(arg));
  } else if (FAILED(env_->NewRandomRWFile(arg.data_file_path_, file_, env_options_).code())) {
    SE_LOG(ERROR, "fail to create data file", K(ret), K(arg));
  } else if (FAILED(init_fallocate_file())) {
    SE_LOG(ERROR, "fail to initialize fallocate datafile", K(ret));
  } else if (FAILED(init_header(arg))) {
    SE_LOG(ERROR, "fail to init header", K(ret), K(arg));
  } else if (FAILED(double_write_header())) {
    SE_LOG(ERROR, "fail to double write header", K(ret));
  } else {
    ...
    offset_heap_.push(1);
    is_inited_ = true;
    SE_LOG(INFO, "success to create data file", K(arg));
  }
...
int DataFile::open(const std::string &file_path)
{
  ...
  if (UNLIKELY(is_inited_)) {
    ret = Status::kInitTwice;
    SE_LOG(WARN, "DataFile has been inited", K(ret), K(file_path));
  } else if (FAILED(env_->FileExists(file_path).code())) {
    SE_LOG(ERROR, "DataFile should been existed", K(ret), K(file_path));
  } else if (FAILED(env_->NewRandomRWFile(file_path, file_, env_options_).code())) {
    SE_LOG(ERROR, "fail to create file", K(ret), K(file_path));
  } else if (FAILED(double_load_header(file_path))) {
    SE_LOG(ERROR, "fail to double load header", K(ret));
  } else if (FAILED(env_->GetFileSize(file_path, &file_size).code())) {
    SE_LOG(ERROR, "fail to get file size", K(ret));
  } else {
    data_file_path_ = file_path;
    total_extent_count_ = file_size / MAX_EXTENT_SIZE;
    is_inited_ = true;
    SE_LOG(INFO, "success to open datafile", K(file_path), K(file_size), K_(total_extent_count));
  }
...
int DataFile::remove()
{
  ...
  if (UNLIKELY(!is_inited_)) {
    ret = Status::kNotInit;
    SE_LOG(WARN, "DataFile should been inited first", K(ret));
  } else if (!is_free()) {
    ret = Status::kErrorUnexpected;
    SE_LOG(WARN, "unexpected error, DataFile not free, can't been remove", K(ret), K_(total_extent_count), K_(free_extent_count));
  } else {
    close();
    if (FAILED(env_->DeleteFile(data_file_path_).code())) {
      SE_LOG(WARN, "fail to delete file", K(ret));
    } else {
      SE_LOG(INFO, "success to delete datafile", K_(data_file_path));
    }
  }
...
int DataFile::close()
{
  ...
  if (UNLIKELY(!is_inited_)) {
    ...
    SE_LOG(INFO, "the file has closed", "file_number", data_file_header_.file_number_);
  } else if (FAILED(file_->Close().code())) {
    SE_LOG(WARN, "fail to close file", K(ret));
  } else {
    SE_LOG(INFO, "success to close file", "file_number", data_file_header_.file_number_);
  }
  return ret;
}
...
int DataFile::recycle(const ExtentId extent_id)
{
  ...
  if (UNLIKELY(!is_inited_)) {
    ...
    ret = Status::kErrorUnexpected;
    SE_LOG(WARN, "unexpected error, the extent is free", K(ret), K(extent_id));
  } else {
...
int DataFile::reference(const ExtentId extent_id, ExtentIOInfo &io_info)
{
  ...
  if (UNLIKELY(!is_inited_)) {
    ret = Status::kNotInit;
    SE_LOG(WARN, "DataFile shoule been inited first", K(ret));
  } else if (UNLIKELY(data_file_header_.file_number_ != extent_id.file_number)
             || UNLIKELY(total_extent_count_ <= extent_id.offset)) {
    ret = Status::kInvalidArgument;
    SE_LOG(WARN, "invalid argument", K(ret), K_(data_file_header), K(extent_id), K_(total_extent_count));
...
int DataFile::shrink(const int64_t shrink_extent_count)
{
  ...
  if (UNLIKELY(!is_inited_)) {
    ...
    ret = Status::kInvalidArgument;
    SE_LOG(WARN, "invalid argument", K(ret), K(shrink_extent_count), K_(free_extent_count));
  } else {
    for (int32_t i = 1; SUCCED(ret) && i <= shrink_extent_count; ++i) {
      if (is_used_extent(total_extent_count_ - i)) {
        ret = Status::kErrorUnexpected;
        SE_LOG(WARN, "this extent should been free", K(ret), K_(total_extent_count), K(i));
      }
    }

    if (SUCCED(ret)) {
      if (FAILED(truncate(shrink_extent_count))) {
        SE_LOG(WARN, "fail to truncate data file", K(ret), K_(data_file_header), K(shrink_extent_count));
      }
    }
  }
...
int DataFile::get_extent_io_info(const ExtentId extent_id, ExtentIOInfo &io_info)
{
  ...
  if (UNLIKELY(!is_inited_)) {
    ret = Status::kNotInit;
    SE_LOG(WARN, "DataFile should been inited first", K(ret), K(extent_id));
  } else if (UNLIKELY(data_file_header_.file_number_ != extent_id.file_number)
      || UNLIKELY(total_extent_count_ <= extent_id.offset)) {
    ret = Status::kInvalidArgument;
...
int DataFile::rebuild()
{
  ...
  if (UNLIKELY(!is_inited_)) {
    ...
    SE_LOG(WARN, "fail to rebuild free extent", K(ret));
  }
...
int DataFile::double_write_header()
{
  ...
  if (TEST_is_double_write_header_failed()) {
    ret = Status::kErrorUnexpected;
    SE_LOG(ERROR, "inject double write header error", K(ret));
    return ret;
  }
#endif
  ...
  if (UNLIKELY(!data_file_header_.is_valid())) {
    ...
    if (data_file_header_.get_serialize_size() >= BACKUP_HEADER_OFFSET) {
      ret = Status::kErrorUnexpected;
      SE_LOG(WARN, "unexpected error, DataFileHeader is too large", K(ret), "header_size", data_file_header_.get_serialize_size());
    } else {
      //serialize master header
      pos = MASTER_HEADER_OFFSET;
      if (FAILED(data_file_header_.serialize(header_buf, header_buf_size, pos))) {
        SE_LOG(WARN, "fail to serialize space header", K(ret));
      }
      ...
      if (SUCCED(ret)) {
        ...
        if (FAILED(data_file_header_.serialize(header_buf, header_buf_size, pos))) {
          SE_LOG(WARN, "fail to serialize datafile header", K(ret));
        }
      }
      ...
      if (SUCCED(ret)) {
        if (FAILED(file_->Write(MASTER_HEADER_OFFSET, Slice(header_buf, header_buf_size)).code())) {
          SE_LOG(WARN, "fail to write header to datafile", K(ret));
        }
      }
    }
  }
...
int DataFile::double_load_header(const std::string &file_path)
{
  ...
  if (IS_NULL(header_buf = reinterpret_cast<char *>(
    ...
    if (FAILED(file_->Read(0, header_buf_size, &header_result, header_buf).code())) {
      SE_LOG(WARN, "fail to read header buf", K(ret));
    } else if (header_buf_size != header_result.size()) {
      ret = Status::kIOError;
      SE_LOG(WARN, "fail to read expect size header", K(ret), K(header_buf_size), "result_size", header_result.size());
    } else if (FAILED(load_header(header_result, MASTER_HEADER_OFFSET, master_header))) {
      SE_LOG(WARN, "fail to load master header", K(ret));
    } else {
      if (DataFileHeader::DATA_FILE_HEADER_VERSION_V1 == master_header.header_version_) {
        ...
        ret = Status::kErrorUnexpected;
        SE_LOG(WARN, "unexpected error, the data file header maybe wrong", K(ret), K(master_header));
      }

      if (SUCCED(ret)) {
        if (FAILED(load_header(header_result, backup_header_offset, backup_header))) {
          SE_LOG(WARN, "fail to load backup header", K(ret));
        } else if (!(master_header == backup_header)) {
          ret = Status::kErrorUnexpected;
          SE_LOG(WARN, "unexpected error, master_header and backup_header mismatch", K(ret), K(master_header), K(backup_header));
        } else {
          ...
          if (DataFileHeader::DATA_FILE_HEADER_VERSION_V1 == master_header.header_version_) {
            ...
            if (FAILED(double_write_header())) {
              SE_LOG(WARN, "fail to double write header", K(ret));
            }
          }
        }
      }
  ...
  if (FAILED(ret)) {
    if (Status::kOk != (tmp_ret = check_garbage_file(file_path, header_result))) {
      SE_LOG(ERROR, "fail to check garbage file", K(ret), K(tmp_ret));
    }
  }
...
int DataFile::load_header(const Slice &header_buf, const int64_t offset, DataFileHeader &data_file_header)
{
  ...
  if (IS_NULL(header_buf.data()) || UNLIKELY(0 == header_buf.size())) {
    ret = Status::kInvalidArgument;
    SE_LOG(WARN, "invalid argument", K(ret), KP(header_buf.data()), "size", header_buf.size());
  } else if (FAILED(data_file_header.deserialize(header_buf.data(), header_buf.size(), pos))) {
    SE_LOG(ERROR, "fail to deserialize space header", K(ret), K(pos));
  } else if (DataFileHeader::DATA_FILE_HEADER_MAGIC_NUMBER != data_file_header.magic_number_) {
    ret = Status::kErrorUnexpected;
    SE_LOG(ERROR, "unexpected error, DataFileHeader's magic number mismatch", K(ret), K(data_file_header));
  }
...
int DataFile::init_fallocate_file()
{
  ...
  if (TEST_is_fallocate_failed()) {
    ret = Status::kErrorUnexpected;
    SE_LOG(ERROR, "inject fallocate file error", K(ret));
    return ret;
  }
#endif
  ...
  if (UNLIKELY(!is_inited_)) {
    ...
    if (need_expand_space()) {
      if (FAILED(expand_space(calc_expand_extent_count()))) {
        SE_LOG(WARN, "fail to expand space", K(ret));
      }
    }
    ...
    if (SUCCED(ret)) {
      if (offset_heap_.empty()) {
        ret = Status::kNoSpace;
        SE_LOG(INFO, "datafile has no space", K_(total_extent_count));
      } else {
        extent_id.file_number = data_file_header_.file_number_;
        extent_id.offset = offset_heap_.top();
        if (is_used_extent(extent_id.offset)) {
          ret = Status::kErrorUnexpected;
          SE_LOG(WARN, "unexpected error, the extent has been used", K(ret), K(extent_id));
          abort();
        } else {
          set_used_status(extent_id.offset);
          offset_heap_.pop();
          dec_free_extent_count(1);
          SE_LOG(DEBUG, "success to allocate extent", K(extent_id));
        }
      }
    }
  }
...
int DataFile::expand_space(int64_t expand_extent_cnt)
{
  ...
  if (UNLIKELY(!is_inited_)) {
    ...
    ret = Status::kInvalidArgument;
    SE_LOG(WARN, "invalid argument", K(ret), K(expand_extent_cnt));
  } else if (FAILED(file_->fallocate(mode, origin_file_size, expand_file_size))) {
    SE_LOG(WARN, "fail to fallocate file", K(ret)/*, K_(data_file_header)*/, K(expand_extent_cnt), K(expand_file_size));
  } else if (FAILED(update_data_file_size(new_total_extent_count))) {
    SE_LOG(WARN, "fail to update data file header", K(ret), K(new_file_size), K(new_total_extent_count));
  } else {
    //add expand offset to heap
    for (int32_t i = origin_total_extent_count; SUCCED(ret) && i < new_total_extent_count; ++i) {
      offset_heap_.push(i);
      SE_LOG(DEBUG, "success to push offset", "file_number", data_file_header_.file_number_, K(i));
    }
    inc_free_extent_count(expand_extent_cnt);
    uint64_t actual_file_size = 0;
    env_->GetFileSize(data_file_path_, &actual_file_size);
    SE_LOG(INFO, "success to expand space", "file_number", data_file_header_.file_number_,
...
int DataFile::truncate(const int64_t truncate_extent_count)
{
  ...
  if (UNLIKELY(!is_inited_)) {
    ...
    ret = Status::kInvalidArgument;
    SE_LOG(WARN, "invalid argument", K(ret), K(truncate_extent_count), K_(total_extent_count));
  } else if (FAILED(file_->ftruncate(new_file_size))) {
    SE_LOG(WARN, "fail to truncate file", K(ret), K_(data_file_header), K(new_file_size));
  } else if (FAILED(update_data_file_size(new_total_extent_count))) {
    SE_LOG(WARN, "fail to update data file size", K(new_file_size), K(new_total_extent_count));
  } else if (FAILED(rebuild_free_extent())) {
    SE_LOG(WARN, "fail to rebuild extent", K(ret), K(new_file_size), K(new_total_extent_count));
  } else {
    uint64_t actual_file_size = 0;
    env_->GetFileSize(data_file_path_, &actual_file_size);
    SE_LOG(INFO, "success to truncate data file", "file_nummber", data_file_header_.file_number_,
...
int DataFile::update_data_file_size(const int64_t total_extent_count)
{
  ...
  if (UNLIKELY(total_extent_count <= 0)) {
    ret = Status::kInvalidArgument;
    SE_LOG(WARN, "invalid argument", K(ret), K(total_extent_count));
  } else {
...
int DataFile::check_garbage_file(const std::string &file_path, const Slice &header_buf)
{
  ...
  if (FAILED(env_->GetFileSize(file_path, &file_size).code())) {
    SE_LOG(WARN, "fail to get file size", K(ret));
  } else if (0 == file_size) {
    garbage_file_ = true;
    SE_LOG(INFO, "garbase file, occupy zero size", K(file_path));
  } else {
    SE_LOG(INFO, "not empty garbage file", K(file_path), K(file_size));
  }
  ...
  if (SUCCED(ret) && !garbage_file_) {
    if (IS_NULL(garbage_header_buf = reinterpret_cast<char *>(
      ...
      if ((0 == (memcmp(garbage_header_buf, header_buf.data(), header_buf.size())))
        ...
        garbage_file_ = true;
        SE_LOG(INFO, "garbage file, header filled with zero", K(file_path), K(file_size));
      } else {
        SE_LOG(INFO, "not empty header garbage file", K(file_path), K(file_size));
      }
    }
  }
