...
int ReplayThreadPool::init() {
  ...
  if(UNLIKELY(inited_)) {
    ...
    SE_LOG(WARN, "ReplayThreadPool init failed", K(ret), K(thread_count_), KP(db_impl_));
  } else {
    ...
    if (FAILED(replay_thread_pool_executor_->initialize(thread_count_))) {
      SE_LOG(WARN, "ReplayThreadPool init failed", K(ret));
      MOD_DELETE_OBJECT(ReplayThreadPoolExecutor, replay_thread_pool_executor_);
    } else {
      SE_LOG(INFO, "replay thread pool init succ", K(thread_count_));
      inited_ = true;
  ...
  if (UNLIKELY(!inited_)) {
    ...
    while (SUCC(ret) && get_submit_task_num() > get_executed_task_num()) {
      ...
      if (FAILED(pre_process_execute())) {
        SE_LOG(WARN, "fail to pre_process_execute while waiting for all tasks executed", K(ret));
        break;
      }
    }
...
int ReplayThreadPool::pre_process_execute(bool need_before_barrier) {
  ...
  if (UNLIKELY(!inited_)) {
    ret = Status::kNotInit;
    SE_LOG(WARN, "pre_process_execute failed", K(ret));
  } else {
    if (db_impl_->check_if_need_switch_memtable()) { // check if need switch memtable
      if (FAILED(set_during_switching_memtable())) {
        SE_LOG(WARN, "set replay thread pool during switching memtable failed", K(ret));
      } else if (FAILED(wait_for_all_threads_suspend())) {
        SE_LOG(WARN, "wait for replay threads suspend failed", K(ret));
      } else {
        ...
        if (FAILED(db_impl_->switch_memtable_during_parallel_recovery(switched_sub_tables))) {
          ...
          SE_LOG(WARN, "notify thread pool switching finished failed", K(ret));
...
int ReplayThreadPool::build_and_submit_task(WriteBatch *write_batch,
  ...
  if (UNLIKELY(!inited_)) {
    ret = Status::kNotInit;
    SE_LOG(WARN, "submit task failed", K(ret));
  } else {
    ...
    if (replay_task == nullptr) {
      ...
      SE_LOG(WARN, "build replay task failed", K(ret));
    } else if (FAILED(pre_process_execute(replay_task->need_before_barrier_during_replay()))) {
      SE_LOG(ERROR, "pre_process_execute before submit_task failed", K(ret));
    } else {
      ...
      if (FAILED(replay_thread_pool_executor_->submit_task(replay_task,
        ...
        SE_LOG(ERROR, "submit replay task into thread pool failed", K(ret));
...
int ReplayThreadPool::stop() {
  ...
  if (UNLIKELY(!inited_)) {
    ret = Status::kNotInit;
    SE_LOG(WARN, "stop replay thread pool fail", K(ret));
  } else {
    if (nullptr != replay_thread_pool_executor_) {
      replay_thread_pool_executor_->stop();
    }
  }
  SE_LOG(INFO, "stopped replay_thread_pool", K(ret));
  return ret;
}
...
int ReplayThreadPool::destroy(uint64_t* replayed_task_num) {
  ...
  SE_LOG(INFO, "replay_thread_pool destroyed", K(has_error), K(replayed_num));
  return ret;
}
...
int ReplayThreadPool::set_error() {
  ...
  if (UNLIKELY(!inited_)) {
    ret = Status::kNotInit;
    SE_LOG(WARN, "set error fail", K(ret));
  } else {
...
int ReplayThreadPool::set_during_switching_memtable() {
  ...
  SE_LOG(INFO, "start switching memtable, replay_thread_pool will suspend", K(ret));
  return ret;
}
...
int ReplayThreadPool::finish_switching_memtable() {
  ...
  SE_LOG(INFO, "finish switching memtable, replay_thread_pool will resume", K(ret));
  return ret;
}
  ...
  if (UNLIKELY(!inited_)) {
    ...
    while (SUCC(ret) &&
      ...
      if (current_timestamp > last_log_wait_time_ &&
        ...
        last_log_wait_time_ = env->NowMicros();
        SE_LOG(WARN, "wait for all threads suspend for too long", "suspend_threads_num", get_suspend_thread_num());
      }
      std::this_thread::yield();
    }
    if (replay_thread_pool_executor_->has_error()) {
      SE_LOG(WARN, "fail to wait for all threads suspend, has error");
      ret = Status::kCorruption;
    }
  }
  return ret;
...
bool ReplayThreadPool::all_threads_stopped() const {
  ...
  if (finished) {
    ...
    if (left_task_num != 0) {
      SE_LOG(INFO, "replay_thread_pool has not executed all tasks", K(left_task_num));
    }
  }
  return finished;
}
...
void ReplayThreadPool::print_replay_thread_pool_status() {
  SE_LOG(INFO, "replay thread pool status", "submit_task_num",
