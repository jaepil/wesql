...
Status WalManager::GetSortedWalFiles(VectorLogPtr& files) {
  ...
  if (!files.empty()) {
    ...
    __SE_LOG(INFO, "Latest Archived log: %" PRIu64,
  ...
  for (auto& log : logs) {
    if (log->LogNumber() > latest_archived_log_number) {
      ...
      __SE_LOG(WARN, "%s already moved to archive",
                     log->PathName().c_str());
    }
  }
...
void WalManager::PurgeObsoleteWALFiles() {
  ...
  if (!s.ok()) {
    __SE_LOG(ERROR, "Can't get current time: %s",
                    s.ToString().c_str());
    assert(false);
    return;
  }
  ...
  if (!s.ok()) {
    __SE_LOG(ERROR, "Can't get archive files: %s", s.ToString().c_str());
    assert(false);
    return;
  }
  ...
  for (auto& f : files) {
    ...
    if (ParseFileName(f, &number, &type) && type == kLogFile) {
      ...
      if (size_limit_enabled) {
        ...
        if (!s.ok()) {
          __SE_LOG(ERROR, "Unable to get file size: %s: %s", file_path.c_str(), s.ToString().c_str());
          return;
        } else {
          if (file_size > 0) {
            ...
            s = env_->DeleteFile(file_path);
            if (!s.ok()) {
              __SE_LOG(WARN, "Unable to delete file: %s: %s", file_path.c_str(), s.ToString().c_str());
              continue;
            } else {
              SE_LOG(INFO, "success to delete wal file", K(file_path));
              MutexLock l(&read_first_record_cache_mutex_);
              read_first_record_cache_.erase(number);
            }
          }
...
Status WalManager::ReadFirstRecord(const WalFileType type,
  ...
  *sequence = 0;
  if (type != kAliveLogFile && type != kArchivedLogFile) {
    __SE_LOG(ERROR, "[WalManger] Unknown file type %s", ToString(type).c_str());
