...
void ReplayTask::run() {
  ...
  if (db_impl_ == nullptr) {
    ...
    SE_LOG(ERROR, "replay log fail", K(ret), K(log_file_number_),
  ...
  SE_LOG(DEBUG, "replay wal record", K(ret), K(log_file_number_), "sequence",
...
int ReplayTask::replay_log() {
  ...
  if (replay_error_) {
    ...
    SE_LOG(ERROR, "replay wal record failed", K(ret), K(log_file_number_),
        "sequence", WriteBatchInternal::Sequence(write_batch_));
    return ret;
  }
#endif
  ...
  if (FAILED(db_impl_->parallel_recovery_write_memtable(*write_batch_,
    ...
    SE_LOG(ERROR, "update max sequence failed", K(ret), K(log_file_number_),
        "sequence", WriteBatchInternal::Sequence(write_batch_));
  }
  return ret;
}
    ...
    switch (tag) {
      ...
      case kTypeCommitXID:
        if (!db_impl->get_prepare_sequence_from_xid_map(xid.ToString(), &prepare_seq)) {
          SE_LOG(INFO, "xid hasn't been prepared before commit", "xid", xid.ToString(true));
      ...
      case kTypeRollbackXID:
        if (!db_impl->get_prepare_sequence_from_xid_map(xid.ToString(), &prepare_seq)) {
          SE_LOG(INFO, "xid hasn't been prepared before rollback", "xid", xid.ToString(true));
