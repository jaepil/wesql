...
TEST_F(PerfContextTest, FullOutputTest) {
  QueryPerfContext *ctx = QueryPerfContext::new_query_context();
  ctx->reset();
  memset((char*)ctx + 8, 0xBE, MAX_TRACE_POINT * 24 + MAX_COUNT_POINT * 16);
  const char *data = nullptr;
  int64_t size = 0;
  ctx->to_string(10 /*total_time*/, data, size);
...
TEST_F(PerfContextTest, TraceOutputTest) {
  ...
  smartengine::monitor::get_tls_query_perf_context()->get_global_trace_info(time, count);
