...
void DumpDBFileSummary(const ImmutableDBOptions& options,
  ...
  if (!s.ok()) {
    if (!s.IsNotFound()) {
      __SE_LOG(ERROR, "Error happened when reading directory:%s, status=%s",
  ...
  for (auto& db_path : options.db_paths) {
    ...
    __SE_LOG(INFO, "SST files in %s dir, Total Num: %" PRIu64 ", files: %s\n",
  ...
  __SE_LOG(INFO, "Write Ahead Log file in %s: %s",
