 ...
 private:
  ...
  void ConvertLogFilesToTables() {
    for (size_t i = 0; i < logs_.size(); i++) {
      ...
      if (!status.ok()) {
        __SE_LOG(WARN, "Log #%" PRIu64 ": ignoring conversion error: %s", logs_[i], status.ToString().c_str());
      }
      ArchiveFile(logname);
    }
  }
  ...
  Status ConvertLogToTable(uint64_t log) {
    struct LogReporter : public log::Reader::Reporter {
      ...
      virtual void Corruption(size_t bytes, const Status& s) override {
        ...
        __SE_LOG(ERROR, "Log #%" PRIu64 ": dropping %d bytes; %s", lognum, static_cast<int>(bytes), s.ToString().c_str());
      }
    };
    ...
    */
    ...
    while (reader.ReadRecord(&record, &scratch)) {
      ...
      if (status.ok()) {
        ...
        __SE_LOG(WARN, "Log #%" PRIu64 ": ignoring %s", log, status.ToString().c_str());
...
//          env_options_, immutable_db_options_);
      ...
      status = BuildTable(
          ...
          *cfd->GetLatestMutableCFOptions(),
          ...
          0);
      __SE_LOG(INFO, "Log #%" PRIu64 ": %d ops saved to Table #%" PRIu64 " %s",
  ...
  void ExtractMetaData() {
    for (size_t i = 0; i < table_fds_.size(); i++) {
      ...
      if (!status.ok()) {
        ...
        __SE_LOG(WARN, "Table #%s: ignoring %s", file_num_buf, status.ToString().c_str());
        ArchiveFile(fname);
      } else {
        tables_.push_back(t);
      }
    ...
    if (status.ok()) {
      ...
      for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
        ...
        if (!ParseInternalKey(key, &parsed)) {
          __SE_LOG(ERROR, "Table #%" PRIu64 ": unparsable key %s", t->meta.fd.GetNumber(), EscapeString(key).c_str());
          continue;
        }
      ...
      __SE_LOG(INFO, "Table #%" PRIu64 ": %d entries %s", t->meta.fd.GetNumber(), counter, status.ToString().c_str());
    }
*/
    return status;
  }
