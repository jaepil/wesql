...
Slice PlainTableIndexBuilder::Finish() {
  ...
  keys_per_prefix_hist_.Add(num_keys_per_prefix_);
  __SE_LOG(INFO, "Number of Keys per prefix Histogram: %s", keys_per_prefix_hist_.ToString().c_str());
...
Slice PlainTableIndexBuilder::FillIndexes(
  ...
  __SE_LOG(DEBUG, "Reserving %" PRIu32 " bytes for plain table's sub_index", sub_index_size_);
  ...
  __SE_LOG(DEBUG, "hash table size: %d, suffix_map length %" ROCKSDB_PRIszt, index_size_, sub_index_size_);
