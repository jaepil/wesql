...
class BlockReadAmpBitmap {
 public:
  explicit BlockReadAmpBitmap(size_t block_size, size_t bytes_per_bit)
    ...
    memset((void *)(bitmap_), 0, bitmap_size * kBytesPersEntry);
    QUERY_COUNT_ADD(smartengine::monitor::CountPoint::READ_AMP_TOTAL_READ_BYTES,
  ...
  void Mark(uint32_t start_offset, uint32_t end_offset) {
    ...
    if (new_useful_bytes > 0) {
      QUERY_COUNT_ADD(
          smartengine::monitor::CountPoint::READ_AMP_ESTIMATE_USEFUL_BYTES,
          new_useful_bytes);
    }
  }
