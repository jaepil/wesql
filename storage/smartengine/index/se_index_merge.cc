...
*/
int SeIndexMerge::merge_file_create(struct merge_file_info& file_info)
{
//  assert(m_merge_file.fd == -1);

  int ret = HA_EXIT_SUCCESS;
  int fd = 0;

  /**If not set se tmpfile, use mysql_tmpdir as default path*/
...
*/
int SeIndexMerge::add(const common::Slice &key, const common::Slice &val, bool &inserted)
{
  ...
  */
  const uint total_offset = SE_MERGE_CHUNK_LEN +
                            m_rec_buf_unsorted->curr_offset +
                            SE_MERGE_KEY_DELIMITER + SE_MERGE_VAL_DELIMITER +
                            key.size() + val.size();
...
*/
int SeIndexMerge::merge_buf_write()
{
  assert(m_merge_file.fd != -1);
  assert(m_rec_buf_unsorted != nullptr);
  assert(m_output_buf != nullptr);
  assert(!m_offset_array.empty());

  /* Write actual chunk size to first 8 bytes of the merge buffer */
  merge_store_uint64(m_output_buf->block,
                     m_rec_buf_unsorted->curr_offset + SE_MERGE_CHUNK_LEN);
  m_output_buf->curr_offset += SE_MERGE_CHUNK_LEN;
  ...
  */
  ...
  for (const auto &rec : m_offset_array) {
    assert(m_output_buf->curr_offset <= m_merge_buf_size);
  ...
  assert(m_output_buf->curr_offset <= m_output_buf->total_size);
...
*/
int SeIndexMerge::merge_heap_prepare()
{
  assert(m_merge_min_heap.empty());
  ...
  */
  ...
  assert(m_merge_file.num_sort_buffers > 0);
  ...
  */
  ...
  for (ulonglong i = 0; i < m_merge_file.num_sort_buffers; i++) {
    ...
    */
    ...
    if (total_size - SE_MERGE_CHUNK_LEN == 0) {
      break;
    }
...
*/
int SeIndexMerge::next(common::Slice *const key, common::Slice *const val)
{
  /*
...
*/
void SeIndexMerge::merge_heap_top(common::Slice *const key,
...
*/
int SeIndexMerge::merge_heap_pop_and_get_next(common::Slice *const key,
                                              common::Slice *const val)
{
  /*
  ...
  */
  assert(!entry->chunk_info->is_chunk_finished());
...
int SeIndexMerge::merge_heap_entry::read_next_chunk_from_disk(File fd)
{
...
int SeIndexMerge::merge_buf_info::read_next_chunk_from_disk(File fd)
{
...
*/
int SeIndexMerge::merge_record_compare(const uchar *const a_block,
...
**/
void SeIndexMerge::merge_read_rec(const uchar *const block,
                                  common::Slice *const key,
                                  common::Slice *const val)
{
  /* Read key at block offset into key slice and the value into value slice*/
  read_slice(key, block);
  read_slice(val, block + SE_MERGE_REC_DELIMITER + key->size());
}

void SeIndexMerge::read_slice(common::Slice *slice,
                              const uchar *block_ptr)
{
  uint64 slice_len;
  merge_read_uint64(&block_ptr, &slice_len);

  *slice = common::Slice(reinterpret_cast<const char *>(block_ptr), slice_len);
}

int SeIndexMerge::merge_heap_entry::read_rec(common::Slice *const key,
                                             common::Slice *const val)
{
...
int SeIndexMerge::merge_heap_entry::read_slice(common::Slice *const slice,
                                               const uchar **block_ptr)
{
  if (!chunk_info->has_space(SE_MERGE_REC_DELIMITER)) {
    return HA_EXIT_FAILURE;
  }

  uint64 slice_len;
  merge_read_uint64(block_ptr, &slice_len);
  if (!chunk_info->has_space(SE_MERGE_REC_DELIMITER + slice_len)) {
    return HA_EXIT_FAILURE;
  }

  *slice =
      common::Slice(reinterpret_cast<const char *>(*block_ptr), slice_len);
  *block_ptr += slice_len;
  return HA_EXIT_SUCCESS;
}

size_t SeIndexMerge::merge_heap_entry::prepare(File fd,
                                               ulonglong f_offset,
                                               ulonglong chunk_size)
{
  ...
  if (res != (size_t)-1) {
    block = chunk_info->block + SE_MERGE_CHUNK_LEN;
  }

  return res;
}

size_t SeIndexMerge::merge_buf_info::prepare(File fd, ulonglong f_offset)
{
  ...
  */
  ...
  curr_offset += SE_MERGE_CHUNK_LEN;
  return total_size;
}

/* Store key and value w/ their respective delimiters at the given offset */
void SeIndexMerge::merge_buf_info::store_key_value(const common::Slice &key,
                                                   const common::Slice &val)
{
  store_slice(key);
  store_slice(val);
}

void SeIndexMerge::merge_buf_info::store_slice(const common::Slice &slice)
{
  /* Store length delimiter */
  merge_store_uint64(&block[curr_offset], slice.size());

  /* Store slice data */
  memcpy(&block[curr_offset + SE_MERGE_REC_DELIMITER], slice.data(),
         slice.size());

  curr_offset += slice.size() + SE_MERGE_REC_DELIMITER;
}

void SeIndexMerge::merge_reset()
{
  /*
...
void SeIndexMerge::get_sample(std::vector<common::Slice>& sample)
{
...
int SeIndexMerge::Bg_merge::prepare_entry(size_t part_id)
{
  if (m_interrupt) {
  ...
  const size_t total_size = m_entries[part_id].prepare(
      ...
      m_se_merge->m_merge_combine_read_size / (m_curr_part + 1));
  if (total_size == (size_t)-1) {
    return HA_ERR_INTERNAL_ERROR;
  }
  return HA_EXIT_SUCCESS;
...
int SeIndexMerge::Bg_merge::next(common::Slice *const key,
                                 common::Slice *const val,
                                 size_t part_id)
{
...
int SeIndexMerge::Bg_merge::write_buf()
{
  if (m_buf.curr_offset > SE_MERGE_CHUNK_LEN) {
    /* Write actual chunk size to first 8 bytes of the merge buffer */
    merge_store_uint64(m_buf.block, m_buf.curr_offset);

    assert(m_buf.curr_offset <= m_buf.total_size);
...
int SeIndexMerge::Bg_merge::se_merge_sort(std::vector<common::Slice>& sample,
                                          THD *mysql_thd)
{
  assert(m_buf.curr_offset == 0);

  common::Slice merge_key;
  common::Slice merge_val;
  m_buf.curr_offset = SE_MERGE_CHUNK_LEN;
  int res = HA_EXIT_SUCCESS;
  while ((res = m_se_merge->next(&merge_key, &merge_val)) == 0) {
    ...
    if (total_offset >= m_buf.total_size ||
        (m_curr_part < sample.size() &&
         m_se_merge->m_comparator->Compare(sample[m_curr_part], merge_key) < 0)) {
      if (m_interrupt) {
        m_exit_interrupt = true;
        return res;
      }
...
int SeIndexMerge::Bg_merge::merge(std::vector<common::Slice>& sample,
                                  THD *mysql_thd)
{
  int res = se_merge_sort(sample, mysql_thd);
  return res;
}
