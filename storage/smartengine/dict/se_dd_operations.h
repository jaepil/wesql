...
enum dd_table_keys
{
  ...
  DD_TABLE_NULL_BYTES,
  /** subtable id of hidden primary key added by smartengine */
  DD_TABLE_HIDDEN_PK_ID,
  /** Sentinel */
  DD_TABLE__LAST
};

/** smartengine private keys for dd::Index */
enum dd_index_keys
{
  DD_INDEX_TABLE_ID,
...
/** smartengine private keys for dd::Column */
enum dd_column_keys
{
...
/** smartengine private key strings for dd::Table. @see dd_table_keys */
extern const char *const dd_table_key_strings[DD_TABLE__LAST];
/** smartengine private key strings for dd::Index. @see dd_index_keys */
extern const char *const dd_index_key_strings[DD_INDEX__LAST];
/** smartengine private key strings for dd::Column, @see dd_column_keys */
extern const char *const dd_column_key_strings[DD_COLUMN__LAST];

class DDOperateHelper
{
public:
  /** Copy the engine-private parts of a table or partition definition
  when the change does not affect smartengine. This mainly copies the common
  ...
  static void dd_copy_private(dd::Table &new_table, const dd::Table &old_table);
  ...
  static inline bool dd_table_has_instant_cols(const dd::Table &table)
  {
  ...
  static void dd_copy_table_columns(dd::Table &new_table, const dd::Table &old_table);
  
  /** Add column default values for new instantly added columns
  @param[in]	old_table	MySQL table as it is before the ALTER operation
  @param[in]	altered_table	MySQL table that is being altered
  @param[in,out]	new_dd_table	New dd::Table
  @param[in]	new_table	New smartengine table object */
  static void dd_add_instant_columns(const TABLE *old_table,
