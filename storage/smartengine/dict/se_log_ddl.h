...
private:
  SeDictionaryManager *m_dict;
  SeDdlManager *m_ddl_manager;
  SeSubtableManager *m_cf_manager;
  SeDropIndexThread *m_drop_subtable_thread;

public:
  SeDdlLogManager()
      : m_dict(nullptr),
        m_ddl_manager(nullptr),
        m_cf_manager(nullptr),
        m_drop_subtable_thread(nullptr)
  {}

  ~SeDdlLogManager() {}

  bool init(SeDictionaryManager *dict_manager,
            SeDdlManager *ddl_manager,
            SeSubtableManager *cf_manager,
            SeDropIndexThread *drop_subtable_thread)
  {
    m_dict = dict_manager;
  ...
  bool write_drop_subtable_log_test(
  ...
  bool write_drop_subtable_log(db::WriteBatch *const xa_batch,
                               uint subtable_id,
                               ulong thread_id,
                               bool is_drop);

  bool write_remove_cache_log(db::WriteBatch *const xa_batch,
                              const std::string &table_name,
                              ulong thread_id);

  bool write_rename_cache_log(db::WriteBatch *const xa_batch,
  ...
  bool replay_remove_cache(std::string &table_name);

  bool replay_rename_cache(std::string &table_name, std::string &dst_table_name);
