...
void DDOperateHelper::dd_copy_private(dd::Table &new_table, const dd::Table &old_table)
{
  uint64 autoinc = 0;
  uint64 version = 0;
  bool reset = false;
  ...
  /**The create option shoule been updated in server layer, and not individually update
  it during instant ddl commit phase.*/
...
void DDOperateHelper::dd_copy_table_columns(dd::Table &new_table, const dd::Table &old_table)
{
...
void DDOperateHelper::dd_add_instant_columns(const TABLE *old_table,
                                             ...
                                             dd::Table *new_dd_table)
{
  ...
  for (uint32_t i = old_table->s->fields; i < altered_table->s->fields; ++i) {
    ...
    in the metadata stored in the smartengine change buffer. */
    ...
    uint field_offset = field->field_ptr() - altered_table->record[0];
    size_t length = 0;
    DD_instant_col_val_coder coder;
    const char *value = coder.encode(field->field_ptr(),col_len, &length);
