 ...
 * $Id:  dummy_learner.cc,v 1.0 04/05/2017 7:35:17 PM
...
int main(int argc, char* argv[]) {
  if (argc < 6) {
    easy_error_log(
        "Usage: ./dummy_learner <clusterId> <listenAddr> <socketFilePath> "
        "<startLogIndex> <endLogIndex>\n");
    easy_error_log(
        "Example: ./dummy_learner 1 127.0.0.1:10005 /tmp/dl.sock 1 0\n");
    return 1;
  }
  ...
  easy_warn_log(
      "Start new learner client with learner address %s and startLogIndex "
      "%llu.\n",
      laddr, startLogIndex);
  if (strcmp(socketFilePath + slen - 5, ".sock") == 0) {
  ...
  while (1) {
    ...
    easy_warn_log(
        "Send log index %llu, contentlen %llu, term %llu, optype %llu.\n",
