 ...
 * $Id:  echo_learner_client.cc,v 1.0 12/20/2016 19:15:45 PM
...
int main(int argc, char* argv[]) {
  if (argc < 4) {
    std::cerr << "Usage: ./echo_learner_client <ip:port> <startLogIndex> "
                 "<clusterId> <readDelay>"
              << std::endl;
    std::cerr
        << "Example: ./echo_learner_client 127.0.0.1:10004 5 1 1000000 (1s) "
        << std::endl;
    return 1;
  }
  ...
  int ret;
  ret = client->open(argv[1], atol(argv[3]), startLogIndex, 1000,
                     2); /* read from index 5, read timeout 1s, cache size 2*/
  // ret= client->openWithTimestamp(argv[1], rl, 1488336538, 10000, 2); /* read
  // from index 5, read timeout 10s, cache size 2*/
  ...
  std::cout << "Real Current Leader ip " << meta.ip << " port " << meta.port
            << std::endl;
  ...
  while (1) {
    ...
    if (ret == 0) {
      usleep(atol(argv[4]));
      std::cout << std::endl
                << "term: " << le.term() << ", index: " << le.index()
                << std::endl;
      if (le.value().length() >= 15)
        std::cout << "value: " << le.value().substr(0, 10) << "..."
                  << std::endl;
      else
