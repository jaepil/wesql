...
uint64_t RDPaxosLog::getLength() {
  std::lock_guard<std::mutex> lg(lock_);
  if (lastLogIndex_ == 0 && firstLogIndex_ == 0) {
    // if record with idx = 0 is deleted, length is zero
    LogEntry entry;
    if (!readEntry(0, &entry)) {
      return 0;
    }
  }
...
void RDPaxosLog::truncateBackward(uint64_t firstIndex) {
  // TODO: why try to keep the entry idx=0
  if (firstIndex < 1) {
    firstIndex = 1;
  }
  ...
  rocksdb::Status status;
  {
    ...
    status = db_->Put(write_options, lastIndexTag, intToString(lastLogIndex_));
    assert(status.ok());
    if (lastLogIndex_ > 0) {
      LogEntry logEntry;
      ...
      if (firstLogIndex_ > 0) {
        firstLogIndex_ = 0;
        rocksdb::Status status =
            db_->Put(write_options, firstIndexTag, intToString(firstLogIndex_));
        assert(status.ok());
      }
...
const size_t RDPaxosLog::DEFAULT_WRITE_BUFFER_SIZE = 4 * 1024 * 1024;
