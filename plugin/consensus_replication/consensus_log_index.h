...
class ConsensusLogIndex {
 public:
  ...
  int clear_all();
  ulonglong get_total_log_size();

  int get_log_file_from_index(uint64 consensus_index, std::string &log_name, uint64 &start_index);
  ...
  int get_first_log_should_purge_by_size(ulonglong purge_target_size,
                                         std::string &log_name, uint64 &index);
  int get_first_log_should_purge_by_time(ulong timestamp /* in seconds */,
                                         std::string &log_name, uint64 &index);
  int update_log_size_by_name(const std::string &log_name, ulonglong log_size);
  uint64 get_start_index_of_file(const std::string &log_name);
  std::string get_last_log_file_name();

  void update_pos_map_by_file_name(std::string &log_name, uint64 consensus_index,
                                  uint64 pos);
