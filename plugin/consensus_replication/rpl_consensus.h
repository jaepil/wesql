...
extern void rpl_consensus_init(bool is_learner, uint64 mock_start_index, ConsensusLogManager  *consensus_log_manager);
...
extern void rpl_consensus_write_log_done_internal(uint64 log_index, bool force_send);

extern uint64 rpl_consensus_wait_commit_index_update(uint64 log_index, uint64 term);
extern void rpl_consensus_set_last_noncommit_dep_index(uint64 log_index);

extern uint rpl_consensus_get_cluster_global_info(std::vector<cluster_global_info> *cgis, bool check_leader);
extern void rpl_consensus_get_cluster_local_info(cluster_local_info *cli);
extern uint rpl_consensus_get_helthy_info(std::vector<cluster_helthy_info> *chis);
extern void rpl_consensus_get_stats(cluster_stats *cs);
extern void rpl_consensus_get_member_changes(std::vector<cluster_memebership_change> *cmcs);

extern void rpl_consensus_set_checksum_mode(bool mode);
extern void rpl_consensus_set_disable_election(bool disable_election, bool disable_stepdown);
extern bool rpl_consensus_get_consensus_async();
extern void rpl_consensus_set_consensus_async(bool consensus_async);
extern void rpl_consensus_set_enable_dynamic_easy_index(bool consensus_dynamic_easyindex);
extern void rpl_consensus_set_replicate_with_cache_log(bool consensus_replicate_with_cache_log);
extern void rpl_consensus_set_compact_old_mode(bool consensus_old_compact_mode);
extern void rpl_consensus_set_force_sync_epoch_diff(uint64 consensus_force_sync_epoch_diff);
extern void rpl_consensus_set_new_follower_threshold(uint64 consensus_new_follower_threshold);
extern void rpl_consensus_set_send_packet_timeout(uint64 consensus_send_timeout);
extern void rpl_consensus_set_send_learner_timeout(uint64 consensus_learner_timeout);
extern void rpl_consensus_set_enable_learner_pipelining(bool consensus_learner_pipelining);
extern void rpl_consensus_set_configure_change_timeout(uint64 consensus_configure_change_timeout);
...
extern int rpl_consensus_set_msg_compress_option(int type, size_t threshold, bool checksum, const std::string &address);
extern void rpl_consensus_set_pipelining_timeout(uint64 consensus_pipelining_timeout);
extern void rpl_consensus_set_large_batch_ratio(uint64 consensus_large_batch_ratio);
...
extern void rpl_consensus_set_optimistic_heartbeat(bool consensus_optimistic_heartbeat);
extern void rpl_consensus_set_sync_follower_meta_interval(uint64 consensus_sync_follower_meta_interval);
...
extern void rpl_consensus_set_enable_auto_reset_match_index(bool consensus_auto_reset_match_index);
extern void rpl_consensus_set_enable_learner_heartbeat(bool consensus_learner_heartbeat);
extern void rpl_consensus_set_enable_auto_leader_transfer(bool consensus_auto_leader_transfer);
extern void rpl_consensus_set_auto_leader_transfer_check_seconds(uint64 consensus_auto_leader_transfer_check_seconds);
...
extern int rpl_consensus_sync_all_learners(std::vector<std::string> &info_vector);
extern int rpl_consensus_configure_member(const std::string &addr, bool force_sync, uint election_weight);
extern void rpl_consensus_force_fix_match_index(const std::string& addr, uint64 match_index);
extern int rpl_consensus_force_single_leader();
extern int rpl_consensus_configure_learner(const std::string& addr, const std::string& source, bool use_applied);
extern int rpl_consensus_force_purge_log(bool local, uint64 index);

extern const char* rpl_consensus_paxos_default_error();
extern const char* rpl_consensus_paxos_error(int error_code);
#endif
