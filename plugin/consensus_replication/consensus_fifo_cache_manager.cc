...
int ConsensusFifoCacheManager::get_log_from_cache(uint64 index, uint64 *term,
  ...
  if (index > log_cache_list[lasti].index /* out of range */) {
    sql_print_error(
        "Consensus fifo log cache out of range , the max index in cache is  "
        "%llu, the required index is %llu",
...
int ConsensusFifoCacheManager::trunc_log_from_cache(uint64 index) {
  ...
  sql_print_information(
      "FIFO before truncate , first index of cache is %llu, cache size is %u",
  ...
  rright = start_point;
  sql_print_information(
      "FIFO after truncate , first index of cache is %llu, cache size is %u",
      log_cache_list[rleft].index,
      (rright + reserve_list_size - rleft) % reserve_list_size);
  mysql_rwlock_unlock(&LOCK_consensuslog_cache);
  return 0;
...
void ConsensusFifoCacheManager::set_lock_blob_index(
  ...
  sql_print_information("ConsensusFifoCacheManager set lock_blob_index to %llu",
                        lock_blob_index_arg);
