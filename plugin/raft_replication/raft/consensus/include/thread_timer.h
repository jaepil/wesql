...
class ThreadTimer {
 public:
  ...
  void init(std::shared_ptr<ThreadTimerService> tts,
            std::shared_ptr<Service> srv, double t, TimerType type,
  ...
  template <typename Callable, typename... Args>
  explicit ThreadTimer(std::shared_ptr<ThreadTimerService> tts, double t,
                       TimerType type, Callable&& f, Args&&... args) {
    init(tts, nullptr, t, type, std::forward<Callable>(f),
         std::forward<Args>(args)...);
  }
  template <typename Callable, typename... Args>
  explicit ThreadTimer(std::shared_ptr<ThreadTimerService> tts,
  ...
  template <typename Callable, typename... Args>
  explicit ThreadTimer(std::shared_ptr<ThreadTimerService> tts,
                       std::shared_ptr<Service> srv, const uint64_t t,
                       TimerType type, Callable&& f, Args&&... args)
 ...
 protected:
  std::shared_ptr<ThreadTimerService> service_;
  std::shared_ptr<Service> srv_;
  double time_;
  TimerType type_;
  ev_timer timer_;
