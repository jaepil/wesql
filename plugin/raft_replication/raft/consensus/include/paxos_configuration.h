...
class Configuration {
 public:
  ...
  virtual ServerRef getServerByAddr(const std::string& addr) = 0;
  ...
  virtual void delAllRemoteServer(const std::string& localStrAddr,
                                  Paxos* paxos) = 0;
  virtual void mergeFollowerMeta(const ::google::protobuf::RepeatedPtrField<
                                 ::alisql::ClusterInfoEntry>&) = 0;
...
class StableConfiguration : public Configuration {
 public:
  ...
  virtual void forEach(const SideEffect& sideEffect, void* ptr) override;
  virtual void forEachLearners(const SideEffect& sideEffect,
                               void* ptr) override;
  virtual bool quorumAll(const Predicate& predicate) const override;
  virtual uint64_t quorumMin(const GetValue& getValue) const override;
  virtual uint64_t forceMin(const GetValue& getValue) const override;
  virtual uint64_t allMin(const GetValue& getValue) const override;
  ...
  virtual ServerRef getServer(uint64_t serverId) override;
  virtual ServerRef getServerByAddr(const std::string& addr) override;
  virtual ServerRef getLearnerByAddr(const std::string& addr) override;
  virtual uint64_t getServerIdFromAddr(const std::string& addr) override;
  virtual const std::vector<ServerRef>& getServers() override {
    return servers;
  }
  virtual const std::vector<ServerRef>& getLearners() override {
    return learners;
  }
  virtual uint64_t getServerNum() const override;
  virtual uint64_t getServerNumLockFree() const override {
    return serversNum.load();
  }
  virtual uint64_t getLearnerNum() const override;
  virtual bool needWeightElection(uint64_t localWeight) override;
  virtual uint64_t getMaxWeightServerId(uint64_t baseEpoch,
                                        ServerRef localServer) override;
  virtual int addMember(const std::string& strAddr, Paxos* paxos) override;
  virtual int delMember(const std::string& strAddr, Paxos* paxos) override;
  virtual int configureLearner(uint64_t serverId, uint64_t source,
                               Paxos* paxos) override;
  virtual int configureMember(const uint64_t serverId, bool forceSync,
                              uint electionWeight, Paxos* paxos) override;
  virtual void addLearners(const std::vector<std::string>& strConfig,
                           Paxos* paxos, bool replaceAll = false) override;
  virtual void delLearners(const std::vector<std::string>& strConfig,
                           Paxos* paxos) override;
  virtual void delAllLearners() override;
  virtual void delAllRemoteServer(const std::string& localStrAddr,
                                  Paxos* paxos) override;
  ...
  virtual std::string membersToString(const std::string& localAddr) override {
    return StableConfiguration::configToString(servers, localAddr);
  }
  virtual std::string membersToString() override {
  ...
  /* append log flow control */
  virtual void reset_flow_control() override;
  virtual void set_flow_control(uint64_t serverId, int64_t fc) override;
