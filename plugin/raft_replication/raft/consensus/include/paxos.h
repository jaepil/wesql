...
class Paxos : public Consensus {
 public:
  ...
  typedef struct Stats {
    ...
    const Stats& operator=(const Stats& other) {
      countMsgAppendLog = other.countMsgAppendLog.load();
      countMsgRequestVote = other.countMsgRequestVote.load();
      countHeartbeat = other.countHeartbeat.load();
      countLeaderCommand = other.countLeaderCommand.load();
      countOnMsgAppendLog = other.countOnMsgAppendLog.load();
      countOnMsgRequestVote = other.countOnMsgRequestVote.load();
      countOnHeartbeat = other.countOnHeartbeat.load();
      countOnLeaderCommand = other.countOnLeaderCommand.load();
      countReplicateLog = other.countReplicateLog.load();
      countLeaderTransfer = other.countLeaderTransfer.load();
      countTruncateBackward = other.countTruncateBackward.load();
      return *this;
    }
  } StatsType;
  ...
  /* FOLLOWER */
  virtual int onAppendLog(PaxosMsg* msg, PaxosMsg* rsp) override;
  virtual int onLeaderCommand(PaxosMsg* msg, PaxosMsg* rsp) override;
  /* CANDIDATE */
  virtual int requestVote(bool force = true) override;
  virtual int onRequestVoteResponce(PaxosMsg* msg) override;
  /* LEADER */
  ...
  virtual uint64_t replicateLog(LogEntry& entry) override {
    return replicateLog_(entry, true);
  }
  virtual int appendLog(const bool needLock) override;
  virtual int appendLogToLearner(
      std::shared_ptr<RemoteServer> wserver = nullptr, bool needLock = false);
  virtual int onAppendLogResponce(PaxosMsg* msg) override;
  virtual int leaderCommand(LcTypeT type,
                            std::shared_ptr<RemoteServer> server = nullptr);
  virtual int onLeaderCommandResponce(PaxosMsg* msg) override;
  virtual int onClusterIdNotMatch(PaxosMsg* msg) override;
  virtual int onMsgPreCheck(PaxosMsg* msg, PaxosMsg* rsp) override;
  virtual int onMsgPreCheckFailed(PaxosMsg* msg) override;
  ...
  void setMinNextEpochCheckSeconds(int checkSeconds) {
    minNextEpochCheckSeconds_ = checkSeconds;
  }
  bool getMinNextEpochCheckSeconds() { return minNextEpochCheckSeconds_; }
  void shutdown();
  void stop();

  virtual int onAppendLogSendFail(PaxosMsg* msg,
                                  uint64_t* newId = nullptr) override;
  ...
  // not safe, only use in some unit tests
  ...
  uint64_t getCommitIndex() {
    // tested by ConsensusAsync.get_commit_index
  ...
  void clearStats() { memset((void*)&stats_, 0, sizeof(StatsType)); }
  ...
  void setMaxDelayIndex4ForceSync(uint64_t val) {
    maxDelayIndex4ForceSync_ = val;
  }
  ...
  uint64_t getServerNum() {
    std::unique_lock<std::mutex> ul(lock_);
    if (shutdown_.load()) return 0;
    return config_->getServerNum();
  }
  uint64_t getLearnerNum() {
    std::unique_lock<std::mutex> ul(lock_);
    if (shutdown_.load()) return 0;
    return config_->getLearnerNum();
  }
  uint64_t getLearnerSource(uint64_t learnerID) {
    std::unique_lock<std::mutex> ul(lock_);
    if (shutdown_.load()) return 0;
    auto learnerServer = config_->getServer(learnerID);
    if (!learnerServer) return 0;
    return learnerServer->learnerSource;
  }
  ...
  bool isConnected(const std::string& addr) {
    ...
    auto server = config_->getServerByAddr(addr);
    if (!server) return false;
    return std::dynamic_pointer_cast<RemoteServer>(server)->connected.load();
  }
  std::string getHost() { return host_; }
  uint getPort() { return port_; }
  bool getConfigureChangePrepared() {
    return ccMgr_.prepared == 1 ? true : false;
  }
  uint64_t getConfigureChangePreparedIndex() {
    return ccMgr_.preparedIndex;
  }
  std::string learnersToString() {
    std::unique_lock<std::mutex> ul(lock_);
    if (shutdown_.load()) return "";
    return config_->learnersToString();
  }
  std::string membersToString() {
    std::unique_lock<std::mutex> ul(lock_);
    if (shutdown_.load()) return "";
    return config_->membersToString();
  }
  std::string membersToString(const std::string& localAddr) {
    std::unique_lock<std::mutex> ul(lock_);
    if (shutdown_.load()) return "";
    return config_->membersToString(localAddr);
  }
  ...
  */
  ...
  bool tryFillFollowerMeta_(
      ::google::protobuf::RepeatedPtrField< ::alisql::ClusterInfoEntry>*
          ciEntries);
  void setAlertLogLevel(AlertLogLevelType level = LOG_WARN) {
 ...
 protected:
  ...
  void checkAndSetkeyScanIndex_(
      const ::google::protobuf::RepeatedPtrField<LogEntry>& entries);
  int getClusterInfoWithNoLock_(std::vector<ClusterInfoType>& cis);
  void stopWithNoLock_();
  ...
  uint64_t maxDelayIndex4ForceSync_;
