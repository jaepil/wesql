  ...
  while (!gStop) {
    ...
    for (i = applyedIndex + 1; i <= paxos->getCommitIndex(); ++i) {
      ...
      if (entry.value().length() < 100)
        std::cout << "====> CommittedMsg:" << entry.value()
                  << ", LogIndex:" << i << std::endl;
      else
        std::cout << "====> CommittedMsg:"
                  << "Too long not display"
                  << ", LogIndex:" << i << std::endl;
    }
    applyedIndex = i - 1;
  }
...
void benchThread(Paxos* paxos, uint64_t threadId, uint64_t num,
  ...
  for (uint64_t i = 1; i < num; ++i) {
    ...
    // else
    // log->append(le);
    if (sleepTime != 0) my_usleep(sleepTime);
  }
  paxos->replicateLog(le);
...
// 2. Compile the example: make benchmark3.
// 3. Run 3 instances of this example:
//    ./consensus/example/benchmark3 127.0.0.1:10001 127.0.0.1:10002
//    127.0.0.1:10003 1 2 1 1000
//    ./consensus/example/benchmark3 127.0.0.1:10001 127.0.0.1:10002
//    127.0.0.1:10003 2 2 1 1000
//    ./consensus/example/benchmark3 127.0.0.1:10001 127.0.0.1:10002
//    127.0.0.1:10003 3 2 1 1000
int main(int argc, char* argv[]) {
  ...
  if (argc < 5) {
    std::cerr << "Usage: ./benchmark3 <ip1:port1> <ip2:port2> <ip3:port3> "
                 "<currentIndex> <client threads> <logtype(012)> <num per "
                 "thread> [value size] [net packet size] [sleeptime]"
              << std::endl;
    std::cerr << "Example: ./benchmark3 127.0.0.1:10001 127.0.0.1:10002 "
                 "127.0.0.1:10003 1 2 1 1000"
              << std::endl;
    return 1;
  }
  ...
  if (index < 1 || index > 3) {
    std::cerr << "currentIndex must be 1, 2 or 3." << std::endl;
    return 1;
  }
  ...
  if (argc >= 7) {
    if (atol(argv[6]) > 0) logType = atol(argv[6]);
  }
  if (argc >= 8) {
    num = atol(argv[7]);
  }
  ...
  std::shared_ptr<PaxosLog> rlog =
      std::make_shared<FilePaxosLog>(logDir, (FilePaxosLog::LogTypeT)logType);
  ...
  if (maxPacketSize != 0) paxos1->setMaxPacketSize(maxPacketSize);
  ...
  for (uint64_t i = 0; i < conc; ++i) {
    ths.push_back(
        new std::thread(benchThread, leader, i, totalQueries, valueSize));
  }
